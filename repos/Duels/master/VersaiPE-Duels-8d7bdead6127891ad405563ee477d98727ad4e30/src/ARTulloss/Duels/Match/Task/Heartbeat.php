...
class Heartbeat extends Task
{
	...
	public function onRun(int $currentTick): void
	{
		...
		if (isset($this->duels->countdownArray[$diff])) {
			...
			foreach ($this->players as $player) {
				$level = $this->duels->getServer()->getLevelByName($this->tempLevel);
				if ($level !== null)
					$level->addSound(new ClickSound($player->getPosition()));
			}
		}
	...
	public function resetAndRemovePlayer(Player $player): void{
		...
		$player->teleport($this->duels->getServer()->getDefaultLevel()->getSpawnLocation());
	}
    /**
     * @return null|Player
     */
	...
	public function addSpectator(Player $player): bool
	{
		...
		$player->teleport(Position::fromObject($this->getArena()->getLocation(), $player->getServer()->getLevelByName($this->getTempLevelName())));
