...
class Duels extends PluginBase{
	...
	private $database;
    /** @var GappleCooldownTask $gappleCooldownTask */
    public $gappleCooldownTask;
	...
	public function registerArenas(): void{
		...
		foreach ($this->duelConfig['Arenas'] as $arenaName => $arena) {
			...
			$server->loadLevel($arena["Level"]);

			$level = $server->getLevelByName($arena["Level"]);
			...
			foreach ($arena["Positions"] as $pos) {
				$bang = explode(':', $pos);
				$positions[] = new Position((float)$bang[0], (float)$bang[1], (float)$bang[2]);
			}
