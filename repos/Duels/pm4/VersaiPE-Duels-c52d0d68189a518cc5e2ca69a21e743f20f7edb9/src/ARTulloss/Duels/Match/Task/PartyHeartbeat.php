...
class PartyHeartbeat extends Heartbeat
{
	...
	protected function sendPlayersScoreboard(): void
	{
		...
		foreach ($players as $index => $player)
			$this->scoreboardTask->setTextForByName($player->getName(), (array) str_replace(['{kit}', '{combat}', '{time}', '{rtime}', '{ping}', '{remaining}', '{total}', '{spectators}'], [$this->kitType, $this->duels->cooldown->combat->getCooldown($players[0]), $this->countdown, Utilities::secondsToReadableTime($this->countdown), $player->getNetworkSession()->getPing(), $playersRemaining, $playersTotal, count((array)$this->getSpectators())], $this->duels->scoreboardArray['Party']));
	}
