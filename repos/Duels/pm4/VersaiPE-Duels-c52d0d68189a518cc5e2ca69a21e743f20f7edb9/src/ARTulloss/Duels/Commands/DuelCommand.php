...
use ARTulloss\Duels\Commands\Sub\Duel\DuelInfo;
...
class DuelCommand extends Command
{
	...
	{
		parent::__construct($name, $owner, CommandExecutor::class);
		...
		if (isset($args[0])) {
			...
			if($sender instanceof Player) {
				switch ($args[0]) {
					...
					case 'info':
					case 'i':
                    $sender->sendMessage(str_replace('{version}', $this->getOwn()->getDescription()->getVersion(), Constants::DUELS_INFO));
						return;
					case 'accept':
					case 'a':
