...
class Listener implements PMListener{
	...
	public function onWorldChange(EntityTeleportEvent $event): void{
		...
		if($player instanceof Player) {
			...
			foreach ($this->duels->duelManager->getAllRunningMatchHeartbeats() as $heartbeat)
				if($heartbeat !== null && ($spectators = $heartbeat->getSpectators()) && $spectators !== null && in_array($player, $spectators, true) && $heartbeat->getTempLevelName() !== $event->getTo()->getWorld()->getDisplayName()) {
					$heartbeat->removeSpectator($player);
					break;
				}
		}
...
//				$event->setCancelled();
    ...
    public function onPacket(DataPacketReceiveEvent $event): void{
	 /*   $pk = $event->getPacket();
	    if($pk instanceof LoginPacket) {
	        // Pocket UI is 1
            // Classic UI is 0
	        $this->playerUITypes[$pk->] = $pk->clientData['UIProfile'];
    ...
    public function createJoinedInventoryContents($inventory, $armorInventory, $pocket): array {
        if($pocket) {
            $firstArmorSlot = 27;
            if(isset($inventory[$firstArmorSlot]) && $inventory[$firstArmorSlot]->getId() !== ItemIds::AIR) {
    ...
    public function sendInventoryMenu(Player $winner, Player $loser, bool $first, array $winnerContents, array $loserContents, float $winnerHealth): void{
        ...
        $task = new ClosureTask(function (int $currentTick) use ($invMenu, $winner, $loser, $first, $winnerContents, $loserContents, $winnerHealth): void {
            ...
            $invMenu->readonly();
            $invMenu->setInventoryCloseListener(function (Player $player, BaseFakeInventory $inventory) use ($winner, $loser, $first, $winnerContents, $loserContents, $winnerHealth) {
        ...
        $this->duels->getScheduler()->scheduleDelayedTask($task, $invMenu->getInventory()->getSendDelay($first ? $winner : $loser));
    }
    /**
