class Embed {
	...
	public addField(
		...
		description =
			description.length >= 2048
				...
				: description;
		this.code.fields = this.code.fields || [];
	...
	public setColor(resolveable: number | string = 0xffffff): this {
		if (typeof resolveable === 'string') {
			...
			if (resolveable.length < 3 || resolveable.length > 10) {
				this.code.color = 0xffffff;
			} else {
				this.code.color = parseInt('0x' + resolveable);
			}
		} else {
			this.code.color = resolveable;
		}
		return this;
	}
	...
	public setTimestamp(date: Date): this {
		this.code.timestamp = date || Date.now();
		return this;
	}
