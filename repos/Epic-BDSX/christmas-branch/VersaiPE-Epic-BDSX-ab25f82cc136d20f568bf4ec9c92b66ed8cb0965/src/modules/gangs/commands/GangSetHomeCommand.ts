...
export default class GangSethomeCommand extends Command {
    ...
    public onRun(player: ExtPlayer, origin: CommandOrigin, params: any) {
        let gang = GangsModule.getGangByPlayer(player.ign) ?? GangsModule.getGangByLeader(player.ign);
        if(!gang) {
        ...
        player.sendMessage(FMT.GREEN + `Successfully set home of the gang (${Math.floor(player.pos.x)}, ${Math.floor(player.pos.y)}, ${Math.floor(player.pos.z)})`);
    }
}