...
class StaffHUD {
    ...
    public function enable(Player $player) {
        ...
        $player->setGenericFlag(Entity::DATA_FLAG_SILENT);
    ...
    public function vanishPlayer(Player $player) {
        if ($player->getGenericFlag(Entity::DATA_FLAG_SILENT) === true) return;
        $player->setNameTag('§c[V]§r ' . str_replace('§c[V]§r ', '', $player->getNameTag()));
        $player->setGenericFlag(Entity::DATA_FLAG_SILENT, true);
        $player->setGamemode(1);
        foreach ($this->plugin->getServer()->getOnlinePlayers() as $pl) {
            if ($player->isOp()) {
                if (!$pl->hasPermission('StaffHUD.staffmode.see_op')) $pl->hidePlayer($player);
    ...
    public function unvanishPlayer(Player $player) {
        if ($player->getGenericFlag(Entity::DATA_FLAG_SILENT) === false) return;
        $player->setNameTag(str_replace('§c[V]§r ', '', $player->getNameTag()));
        $player->setGenericFlag(Entity::DATA_FLAG_SILENT, false);
        foreach ($this->plugin->getServer()->getOnlinePlayers() as $pl) {
            if ($player->isOp()) {
    ...
    public function handleMove($event) {
        ...
        foreach ($this->locked as $mod=>$p) {
            if ($p === $player->getName()) {
                $mod = $this->plugin->getServer()->getPlayer($mod);
                if (!$mod) continue;
                $distance = $this->getDistance($mod, $player);

                if ($distance >= 10) {
                    $mod->teleport(new Position($player->x, $player->y, $player->z, $player->getLevel()));
    ...
    public function handleDamage($event) {
        ...
        if (isset($this->enabled[$ent->getName()]) && ($ent->getGenericFlag(Entity::DATA_FLAG_SILENT) === true)) {
    ...
    public function handleJoin($event) {
        ...
        foreach ($this->enabled as $p=>$time) {
            $pl = $this->plugin->getServer()->getPlayer($p);
            if (!$pl) {
